...
history = [-1 for i in range(len(allcommands))]
...
def run_program():
    ...
    while(1):
        if(history[index] != -1):
            ...
            history[index] = time
            exe_instruction(allcommands[index])
            time += 1


run_program()

print("naar loop har accumulator vaerdien: ", accumulator)

def find_exit():
    global index
    global accumulator
    maxindex = len(allcommands)
    index = 0;
    accumulator = 0;
    while(1):
        if(history[index] != -2 and allcommands[index][0:3] != "acc"):
            saveindex = index
            saveacc = accumulator
            history[index] = -2

            if(allcommands[index][0:3] == "jmp"):
                tmp_instruct = "nop" + allcommands[index][3:]
            elif(allcommands[index][0:3] == "nop"):
                tmp_instruct = "jmp" + allcommands[index][3:]
            else:
                print("ERROR in find_exit...")
                return

            exe_instruction(tmp_instruct)

            while(1):
                if(index > maxindex):
                    break;
                if(index == maxindex):
                    print("Struck gold by changing at index ", saveindex)
                    print("Accumulator would now be: ", accumulator)
                    return;
                if(history[index] != -1):               # no good...
                    history[index] = -2
                    break;
            ...
            # resetting index and accumulator
            index = saveindex
            accumulator = saveacc
        else:
            history[index] = -2
