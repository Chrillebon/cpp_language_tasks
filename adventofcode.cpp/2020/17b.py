...
# level op...
# [w][z][y][x]
pocketdim = [[[["." for i in range(startwidth+2*6)] for o in range(startheight+2*6)] for u in range(13)] for e in range(13)]

for i in range(startheight):
    for o in range(startwidth):
        pocketdim[6][6][6+i][6+o] = mypocket[i][o]

def update_single(x,y,z,w):
    global pocketdim
    active_close = 0
    for e in range(max(0,w-1),min(13,w+2)):
        for i in range(max(0,z-1),min(13,z+2)):
            for o in range(max(0,y-1),min(12+startheight, y+2)):
                for u in range(max(0,x-1),min(12+startwidth, x+2)):
                    if(pocketdim[e][i][o][u] == "A" and (e != w or i != z or o != y or u != x)):
                        active_close += 1
    if(pocketdim[w][z][y][x] == "A" and (active_close == 2 or active_close == 3)):
        #print("Keeping active location: ",x-6-1, ", ",y-6-1,", in layer ", z-6)
        return 1
    elif(pocketdim[w][z][y][x] == "A"):
        #print("Deactivating location: ",x-6-1, ", ",y-6-1,", in layer ", z-6)
        return 0
    elif(pocketdim[w][z][y][x] == "." and active_close == 3):
