...
void com(int id)
{
  ...
  else if(parameter[0] == 3) //Tager input
  {
    ...
    {
      list[whatnum(parameter[1]+1, id+1)] = xy.first; //x
      place = 1;
      //std::cout << "tester: " <<list[whatnum(parameter[1]+1, id+1)]<< ", ";
    }
    else
    {
      list[whatnum(parameter[1]+1, id+1)] = xy.second; //y
      place = 0;
      //std::cout << list[whatnum(parameter[1]+1, id+1)] << '\n';
    }
...
int main()
{
  while(1)
  {
    if(tryout())
    {
      if(done())
      {
        std::cout << "færdig, resultat er: " <<(xy.first-99)*10000+xy.second<< '\n';
        return 0;
      }
      else
      {
        xy.first++;
        //std::cout << "trying x=" <<xy.first<< '\n';
      }
    }
    else
    {
