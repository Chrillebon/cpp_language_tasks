...
void cdist(int a,int b)
{
  ...
  mindst = min(mindst, abs(a-cx)+abs(b-cy));
  return;
}

void secondcheck(int nu, int xx, int yy)
{
  if(nu == -1 && map[yy][xx] > 0)
  {
    mindst = min(mindst, abs(steps)+abs(map[yy][xx]));
    std::cout << "kryds ved: " <<xx<<" og "<<yy<<" med dist: "<<abs(steps)+abs(map[yy][xx])<< '\n';
  }
  return;
}
...
void input(int nu)
{
  ...
  //std::cout << "test" << '\n';
  int ekstra = 1;
  for(int i=0;i<size[nu-1];i+=3+ekstra)
  {
    dir = com[nu-1][i];
    amo = com[nu-1][i+1]-48;
    ekstra = 1;
    while(com[nu-1][i+1+ekstra] != ',')
    {
      amo*=10;
      amo+=com[nu-1][i+1+ekstra]-48;
      ekstra++;
    }
    ekstra--;
    //std::cout << dir <<" og "<<amo<< '\n';
