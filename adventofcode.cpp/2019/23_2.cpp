...
void restart()
{
  for(int i=0;i<50;i++)
  {
    ...
    {
      list[i][o] = start[o]; //mapping value to index for every computer
    }
  }
  return;
}
...
long long whatnum(int par, long long id)
{
  if(par == 0)
  {
    if(list[comp].count(id) == 0) //Hvis den peger på én der ikke findes, peger den på 0:
    {
      return list[comp][0];
    }
    else if(list[comp].count(list[comp][id]) == 0) //Hvis den der peges på peger ud i intet, returnerer den 0
    {
      return 0;
    }
    else
    {
      return list[comp][list[comp][id]];
    }
  }
  else if(par == 1)
  {
    if(list[comp].count(id) == 0)
    {
      return 0;
    }
    else
  ...
  {
    return whatnum(1, list[comp][id]+base[comp]);
  }
  else if(par == 3)
  {
    return list[comp][id]+base[comp];
  }
}
...
void com(int id)
{
  ...
  if(parameter[0] == 1) //Ligger sammen
  {
    ...
    list[comp][whatnum(parameter[3]+1, id+3)] = whatnum(parameter[1], id+1) + whatnum(parameter[2],id+2);
  ...
  {
    if(change_next) //Nu går vi videre til næste computer
    {
      change_next = 0;
      next_comp();
      return; //Sluk computeren midlertidligt, samler op igen, hvor den var når vi kommer tilbage til den.
    }
    if(iord[comp] == 0) //Computeren skal startes
    {
      std::cout << comp << " er nu startet" << '\n';
      list[comp][whatnum(parameter[1]+1, id+1)] = comp;
      iord[comp] = 1;
    }
    else //Den skal se, om der er noget den skal modtage
    {
      if(idle >= 100 && comp == 0 && NAT.first != 0) //Nu spytter den idle input ind
      {
        if(iord[255] == 0)
        {
          std::cout << "IDLE" << NAT.first<<", ";
          list[comp][whatnum(parameter[1]+1, id+1)] = NAT.first;
          iord[255] = 1;
        }
        else if(iord[255] == 1)
        {
          if(NAT.second == lastNAT)
          {
            std::cout << "First repeat: " <<NAT.second<< '\n';
            list[comp][where[comp]] = 99; //Dræber den her
            return;
          }
          std::cout << NAT.second << '\n';
          list[comp][whatnum(parameter[1]+1, id+1)] = NAT.second;
          iord[255] = 0;
          idle = 0;
          lastNAT = NAT.second;
        }
      }
      else if(instructions.count(comp) == 0) //Ikke noget at modtage
      {
        list[comp][whatnum(parameter[1]+1, id+1)] = -1;
        //std::cout << "-" << '\n';
        change_next = 1;
      }
      else if(instructions[comp].front().first != -999) //Bruger tallet -999 på førstepladsen til at holde styr på, om jeg har læst den
      {
        std::cout << "modtager tal: " <<instructions[comp].front().first<< '\n';
        list[comp][whatnum(parameter[1]+1, id+1)] = instructions[comp].front().first;
        instructions[comp].front().first = -999;
      }
      else if(instructions[comp].front().first == -999) //Anden del af input
      {
        std::cout << "derefter : " <<instructions[comp].front().second<< '\n';
        list[comp][whatnum(parameter[1]+1, id+1)] = instructions[comp].front().second;
        instructions[comp].pop();
        if(instructions[comp].size() == 0)
        {
          instructions.erase(comp);
        }
        idle = 0;
      }
    }
  ...
  {
    change_next = 1; //Når den har skrevet, skal den næste gang den modtager, skifte til næste computer.
    out.push_back(whatnum(parameter[1], id+1));
    if(out.size() == 3)
    {
      if(out[0] == 255) //Skriver til NAT
      {
        NAT = {out[1],out[2]};
      }
      else
      {
        instructions[out[0]].push({out[1],out[2]});
      }
      std::cout << "skrev til: " <<out[0]<<" med "<<out[1]<<", "<<out[2]<< '\n';
      out.clear();
    }
    where[comp] += 2;
  }
  else if(parameter[0] == 5) //Hop, hvis pointer == 1
  {
    ...
    {
      where[comp] = whatnum(parameter[2], id+2);
    }
    else
    {
      where[comp] += 3;
  ...
  else if(parameter[0] == 9) //Ændrer basen
  {
    ...
    base[comp] += whatnum(parameter[1], id+1);
    where[comp] += 2;
  }
  else
  {
