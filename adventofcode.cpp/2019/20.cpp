...
vector<vector<int>> visited(129, vector<int>(121));
queue<pair<int, pair<int,int>>> nxt;
...
void display()
{
  for(int i=0;i<119;i++)
  {
    for(int o=0;o<128;o++)
    {
      if(board[o][i] == '.' || board[o][i] == '#')
      {
        if(o == start.first && i == start.second)
        {
          std::cout << "@";
        }
        else if(o == final.first && i == final.second)
        {
          std::cout << "X";
        }
        else
...
void move()
{
  int steps = nxt.front().first;
  int x = nxt.front().second.first;
  int y = nxt.front().second.second;
  nxt.pop();
  if(board[x][y] != '.' || (visited[x][y] <= steps && steps > 0 && visited[x][y] != 0)) //Ikke muligt, eller bedre fundet
  {
    return;
  }
  ...
  {
    nxt.push({steps+1,portals[{x,y}]});
  }
  visited[x][y] = steps;
  nxt.push({steps+1, {x+1,y}});
  nxt.push({steps+1, {x-1,y}});
  nxt.push({steps+1, {x,y+1}});
  nxt.push({steps+1, {x,y-1}});
  return;
}
...
int main()
{
  ...
  all_portals();
  nxt.push({0,{start.first,start.second}});
  while(!nxt.empty())
  {
    move();
  }
  //display();
  std::cout << "best result is: " <<visited[final.first][final.second]<< '\n';
  return 0;
}
