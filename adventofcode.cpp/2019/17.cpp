...
void intersection()
{
  for(int i=1;i<56;i++)
  {
    for(int o=1;o<56;o++)
    {
      if(board[o][i] == '#')
      {
        if(board[o-1][i] == '#' && board[o+1][i] == '#' && board[o][i-1] == '#' && board[o][i+1] == '#')
        {
          cross.push_back({o,i});
        }
      }
...
void com(int id)
{
  ...
  else if(parameter[0] == 4) //Skriver til skærmen
  {
    ...
    char disp = whatnum(parameter[1], id+1);
    std::cout << disp;
    if(disp != 10)
    {
      board[place % 57][place / 57] = disp; //place giver både x og y-værdien ved dens størrelse;
      place++;
    }
...
int main()
{
  ...
  {
    res += cross[i].first*cross[i].second;
  }
  std::cout << "result: " <<res<< '\n';
  return 0;
}
