...
public:
  ...
  int OTTS(string a, int d) //Orbital Transfers to Santa
  {
    if(planets[a].first < 0) //end, reached close point
    {
      return d+abs(planets[a].first)-1;
    }
    else if(a == "COM") //first time through
    {
      return OTTS("SAN", 0);
    }
    else //recursion
    {
      if(d < 0) //first run
      {
        planets[a].first = d+2; //The object you are orbitting at 0
        return OTTS(parent[a], d-1);
      }
      else
      {
        planets[a].first = d-1; //The object santa is orbitting at 0
        return OTTS(parent[a], d+1);
      }
    }
  }
...
int main()
{
  ...
  std::cout << "dist to santa: " <<solar_system.OTTS("YOU", -1)<< '\n';
  return 0;
}
