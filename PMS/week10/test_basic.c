...
int main(void) {
  ...
  pv = malloc_vector(10);
  print_vector(pv);
  assert(write_vector("test_vector.txt",pv) == MATRIX_IO_SUCCESS);
  free_vector(pv);
  pv = NULL;
  ...
  print_matrix(pm);
  assert(write_matrix("test_matrix.txt",pm) == MATRIX_IO_SUCCESS);
  free_matrix(pm);
  pm = NULL;
  ...
  assert(write_sparse_triplet("test_sparse_triplet.txt",pst) == MATRIX_IO_SUCCESS);
  ...
  assert(write_vector("test.txt",NULL) == MATRIX_IO_ILLEGAL_INPUT);
  assert(write_matrix("test.txt",NULL) == MATRIX_IO_ILLEGAL_INPUT);
  assert(write_sparse_triplet("test.txt",NULL) == MATRIX_IO_ILLEGAL_INPUT);
