...
int main()
{
  ...
  while(!cross.empty()) //All rooms
  {
    ...
    if(map[q.front().first][q.front().second][1] == 0) //if unvisited
    {
      ...
      //If the wall is south or west, flip first and second, so crossing is north or south:
      if(cross.top().first.first > cross.top().second.first || cross.top().first.second < cross.top().second.second)
      {
        listwalls[rooms][0] = cross.top().second.first; // x of first room
        listwalls[rooms][1] = cross.top().second.second; // y of first room
        if(cross.top().second.first == cross.top().first.first) //If they have the same x-coor, then the wall is north (flipped) reletive to the first room
        {
      ...
      else //If wall crossing is north or east
      {
